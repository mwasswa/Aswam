@extends('default')

@section('content')
    <div class="col-lg-12 margin-10">
    <fieldset>
        <legend><font color="green"><i class="fa fa-user-plus"></i> Create Account Category</font></legend>
        {!! Form::open(array('action' => 'Hello@accounts','class'=>'form-inline')) !!}
            <div class="form-group margin-10">
                <input type="text" class="form-control" id="email" placeholder="Name" name="name"/>
            </div>
            <div class="form-group margin-10">
                <input type="text" class="form-control" id="email" placeholder="Contact Person" name="contact_person"/>
            </div>
            <div class="form-group margin-10">
                <input type="text" class="form-control" id="pwd" placeholder="Description" name="description"/>
            </div>
            <div class="form-group margin-10">
                <input type="text" class="form-control" id="email" placeholder="Contact Email" name="contact_email"/>
            </div>
            <div class="form-group margin-10">
                <input type="text" class="form-control" id="email" placeholder="Contact Phone" name="contact_phones"/>
            </div>
            <div class="form-group margin-10">
                <select name="status" class="form-control">
                    <option value=""><b>-Select Status-</b></option>
                    <option value="1">Active</option>
                    <option value="0">Inactive</option>
                </select>
            </div>
            <div class="form-group margin-10">
                <input type="submit" class="btn btn-primary" value="Save" name="add_content"/>
            </div>
        {!! Form::close() !!}
    </fieldset>
        </div>

    <div class="col-lg-12 margin-10">
        <fieldset>
            <legend></legend>
        <?php
        $users_arr = $accounts;
        //$data[$id][$username][$firstname][$role]= $lastname;
        $users = '<table class="table table-responsive table-bordered table-striped">
        <thead>
            <tr>
                <th>#</th>
                <th>Account Category</th>
                <th>Description</th>
                <th>Contact Pesrson</th>
                <th>Contact Phone</th>
                <th colspan="2">Actions</th>
                <!--<th><i class="fa fa-plus-circle"></i></th>-->
            </tr>
        </thead>
        <tbody>';
        if (is_array($users_arr) && count($users_arr)) {
            $i = 1;
            foreach ($users_arr as $pos => $info) {
                extract($info);
                            $users .= "<tr>
                <td>$i</td>
                <td>$name</td>
                <td>$description</td>
                <td>$contact_person</td>
                <td>$contact_phones</td>
                <td class='green-font'><a href='index.php/welcome/edit_user/$id'><i class='fa fa-edit'></i></a></td>
                <td class='red-font'><a href='index.php/welcome/delete_user/$id'><i class='fa fa-trash'></i></a></td>
            </tr>";
                $i++;
            }
            $users .="</tbody></table>";
            echo $users;
        }
        ?>
        </fieldset>
    </div>
@stop